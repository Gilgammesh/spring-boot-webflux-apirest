package com.santander.springbootwebfluxapirest.controller;
...
public class ProductoController {
    ...
    public Mono<ResponseEntity<Producto>> upload(@PathVariable String id, @RequestPart FilePart file) {
        return service.findById(id).flatMap(p -> {
            ...
            return file.transferTo(new java.io.File(path + p.getFoto())).then(service.save(p));
    ...
    public Mono<ResponseEntity<Producto>> crearConFoto(Producto producto, @RequestPart FilePart file) {
        ...
        return file.transferTo(new java.io.File(path + producto.getFoto())).then(service.save(producto))
                .map(p -> ResponseEntity.created(
                        URI.create("/api/productos/".concat(p.getId()))).contentType(MediaType.APPLICATION_JSON)
                        .body(p));
    }
